#!/usr/bin/env python3
"""Fusion v6: Fixes — front-hemisphere orbit, sharper B, better C visibility."""
...
def uv_colors(mesh_path, tex_path, N):
    m=trimesh.load(mesh_path,force='mesh')
    pts,fi=trimesh.sample.sample_surface(m,N)
    faces=m.faces[fi]; uv=m.visual.uv
...
def textured_g(mesh_path, tex_path, world_pos, world_size, N=15000, opacity=3.0):
    p,c=uv_colors(mesh_path,tex_path,N)
...
# B: smaller (world_size 1.2) for sharper appearance
b=textured_g('/root/CV/task1/outputs/object_b/threestudio/save/model.obj',
    '/root/CV/task1/outputs/object_b/threestudio/save/texture_kd.jpg',
    [-0.7,-0.3,0.3], 1.2, N=12000, opacity=3.5)

# C: higher density + opacity for poor-quality mesh
c=textured_g('/root/CV/task1/outputs/object_c/magic123/magic123_final.obj',
    '/root/CV/task1/outputs/object_c/magic123/albedo.png',
    [0.15,-0.35,-0.3], 1.2, N=18000, opacity=4.5)
...
mg.active_sh_degree=3; mg.max_sh_degree=3
print(f"Total: {mg._xyz.shape[0]:,} Gaussians")
del bg,a,b,c; torch.cuda.empty_cache(); gc.collect()

# ═══ Render — correct camera side (bonsai visible from negative Z) ═══
N=120
# Camera sweeps left-right at z=-3.5 (correct side for bonsai visibility)
xs=np.linspace(-2.5,2.5,N//2).tolist()+np.linspace(2.5,-2.5,N//2).tolist()

print(f"\nRendering {N} frames (correct side, z=-3.5)...")
for i,x in enumerate(xs):
    z=-3.5; h=0.4+0.15*np.sin(i*np.pi/60)
    eye=np.array([x,h,z])
    ...
    pil=Image.fromarray((img*255).astype(np.uint8)).resize((1024,768),Image.LANCZOS)
