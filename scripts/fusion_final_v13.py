#!/usr/bin/env python3
"""Fusion v13: A scaled down (1.0), tight visible arc 240°-300°, 50° FoV."""
...
# A: Lego — SCALED DOWN (×1.0 instead of ×1.5), far left
a=load_ply('/root/CV/task1/outputs/object_a/2dgs/point_cloud/iteration_30000/point_cloud.ply')
ca=a._xyz.mean(dim=0)
a._xyz=(a._xyz-ca)*1.0+torch.tensor([-3.0,-0.3,1.5],dtype=torch.float32,device='cuda')
a._scaling=a._scaling*1.0
...
# Tight visible arc: 240°-300° (≤15% black), R=3.5, smooth back-forth
N=120; R=3.5; START=np.radians(240); END=np.radians(300)
angles=[START+(END-START)*(0.5-0.5*np.cos(2*np.pi*i/N)) for i in range(N)]
LOOK=np.array([-0.9,-0.4,1.5])

print(f"\nRendering {N} frames (240°-300° visible arc, A scale=1.0)...")
for i,ang in enumerate(angles):
    eye=np.array([R*np.cos(ang), 0.4+0.2*np.sin(2*ang), R*np.sin(ang)])
    cc=gs_cam(eye,LOOK)
