#!/usr/bin/env python3
"""Fusion v11: Camera centered at 270° sweet spot, objects pushed back to Z~1.5."""
...
# Load — objects pushed BACK (Z~1.3-1.8, deeper into bonsai scene)
print("Loading...")
...
a._xyz=(a._xyz-ca)*1.5+torch.tensor([0.8,-0.35,1.5],dtype=torch.float32,device='cuda')
a._scaling=a._scaling*1.5

br=load_mesh('/root/CV/task1/outputs/object_b/threestudio/save/model.obj',
    '/root/CV/task1/outputs/object_b/threestudio/save/texture_kd.jpg',[-0.7,-0.3,1.3],0.8)
cr=load_mesh('/root/CV/task1/outputs/object_c/magic123/magic123_final.obj',
    '/root/CV/task1/outputs/object_c/magic123/albedo.png',[0.3,-0.35,1.8],0.8)
print(f"BG:{bg._xyz.shape[0]:,} A:{a._xyz.shape[0]:,}")

# Orbit centered at 270° (best angle), ±50° sweep, R=3.0
N=120; R=3.0; CENTER_ANG=np.radians(270); SWING=np.radians(60)
angles=[CENTER_ANG+SWING*np.sin(2*np.pi*i/N) for i in range(N)]

print(f"\nRendering {N} frames (orbit centered at 270°±60°)...")
for i,ang in enumerate(angles):
    x=R*np.cos(ang); z=R*np.sin(ang); y=0.4+0.2*np.sin(2*ang)
    eye=np.array([x,y,z])
    look_at=np.array([0.0,-0.4,1.5])  # look deeper into scene
